...
class TLG6BitStream
{
	...
	tTVPMemoryStream * OutStream; // output stream
	unsigned char *Buffer; // output buffer
	long BufferCapacity; // output buffer capacity

public:
	TLG6BitStream(tTVPMemoryStream * outstream) :
		OutStream(outstream),
		BufferBitPos(0),
		BufferBytePos(0),
		Buffer(NULL),
...
static void WriteInt32_memstream(long num, tTVPMemoryStream *out)
{
	char buf[4];
	buf[0] = num & 0xff;
	buf[1] = (num >> 8) & 0xff;
...
//				ftfreq[ft]++;
			...
			for(int c = 0; c < colors; c++)
			{
				...
				bitlength |= (method << 30);
				WriteInt32_memstream(bitlength, memstream);
				bs.Flush();
			}
	...
	if( meta ) {
		struct MetaDictionaryEnumCallback : public tTJSDispatch {
			...
			tjs_error TJS_INTF_METHOD FuncCall(tjs_uint32 flag, const tjs_char * membername,
				...
				// push items
				ttstr value = *param[0];
				tjs_nchar* value0 = new tjs_nchar[value.GetNarrowStrLen() + 1];
				value.ToNarrowStr(value0, value.GetNarrowStrLen() + 1);
				Tags.push_back( std::string(value0) );
				delete value0;
				value = *param[2];
				tjs_nchar* value2 = new tjs_nchar[value.GetNarrowStrLen() + 1];
				value.ToNarrowStr(value2, value.GetNarrowStrLen() + 1);
				Tags.push_back( std::string(value2) );
				delete value2;
				if(result) *result = (tjs_int)1;
				return TJS_S_OK;
			}
